...
function ShowCard1({ Data,CUser }) {
 ...
 const [isFollowed, setIsFollowed] = React.useState(false);
  ...
  console.log("ProfileUser",CUser?._id);
    ...
    const handleLike = async (id) => {
      ...
      if(Data.quote){
        ...
        const res = await axios.post(`http://localhost:4000/api/v1/like/couplet`, { coupletId: id },{withCredentials:true});
        console.log(res.data);
      }
  ...
  return (
    ...
    <div className="w-full h-[360px] flex justify-center items-center text-3xl " style={{backgroundImage:`url(${Data?.BgImageUrl})`,backgroundAttachment:"scroll",backgroundPosition:"center",backgroundRepeat:"no-repeat",backgroundSize:"cover"}}>
   ...
   onClick={()=>{handleLike(Data?._id)}}
    ...
    <h1>{likeCount}</h1>
     ...
     <GiSaveArrow className="text-4xl" />
    </div>
    </div>
    </>
  );
